...
function EmployeeCheckInOut({ employeeId }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://cws-tms-backend.vercel.app/attendance/today/${employeeId}`, {
  ...
  const handleCheckIn = async () => {
    const res = await axios.post(
      `https://cws-tms-backend.vercel.app/attendance/checkin/${employeeId}`,
      {},
      { headers: { Authorization: `Bearer ${token}` } },
    );
    setAttendance(res.data);
  ...
  const handleCheckOut = async () => {
    const res = await axios.post(
      `https://cws-tms-backend.vercel.app/attendance/checkout/${employeeId}`,
      {},
      { headers: { Authorization: `Bearer ${token}` } },
    );
    setAttendance(res.data);
