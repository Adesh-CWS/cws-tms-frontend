...
useEffect(() => {
  const fetchProjects = async () => {
    try {
      ...
      const res = await axios.get(`https://cws-tms-backend.vercel.app/${userId}/projects`);
...
async function fetchRequiredDetails() {
  try {
    // Tasks
    const taskResponse = await axios.get(`https://cws-tms-backend.vercel.app/tasks/${user._id}`);
    ...
    const projectsResponse = await axios.get(
      `https://cws-tms-backend.vercel.app/${user._id}/projects`
    );
...
const calculateStatus = (project) => {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      ...
      try {
        const response = await fetch(`https://cws-tms-backend.vercel.app/${user._id}/members`);
  ...
  const delayedTasks = employeesTasks.filter((t) => {
    ...
    const dueDate = new Date(t.dateOfExpectedCompletion);
    dueDate.setHours(0, 0, 0, 0);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
