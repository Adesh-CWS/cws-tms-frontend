...
function ManagerReportTMS({ user }) {
  ...
  const handleDonutClick = (statusName) => {
    ...
    if (statusName.toLowerCase() === "delayed") {
      filtered = allTasks.filter((task) => {
        ...
        const d = task.dateOfTaskAssignment ? new Date(task.dateOfTaskAssignment) : null;
      ...
      filtered = allTasks.filter((task) => {
        const statusMatch = task?.status?.name?.toLowerCase() === statusName.toLowerCase();
        if (!statusMatch) return false;
  ...
  useEffect(() => {
    axios
      .get("https://cws-tms-backend.vercel.app/managers/list")
  ...
  const taskStatusChartData = useMemo(() => {
    ...
    filteredTasks.forEach((task) => {
      ...
      } else if (counts[statusName] !== undefined) {
        counts[statusName]++;
      }
    });
  ...
  const TaskStatusTooltip = ({ active, payload }) => {
    ...
    if (statusName === "Delayed") {
      ...
      filteredTasks = allTasks.filter((task) => {
        const statusMatch = task?.status?.name?.trim().toLowerCase() === statusName.trim().toLowerCase();
        if (!statusMatch) return false;
    ...
    filteredTasks.forEach((task) => {
      const empName = task?.assignedTo?.name || task?.employeeName || "Unassigned";
  ...
  //     try {
  //       const res = await axios.get(`https://cws-tms-backend.vercel.app/tasks/${managerId}`);
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const userRes = await axios.get("https://cws-tms-backend.vercel.app/me", {
        ...
        const res = await axios.get(
          `https://cws-tms-backend.vercel.app/api/projects/manager/${managerId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  //       `https://cws-tms-backend.vercel.app/employees/manager/${user._id}`,
  //     );
  //     const employeeList = empResponse.data.employees;
  //     const taskResponse = await axios.get("https://cws-tms-backend.vercel.app/task/getall");
  ...
  //       `https://cws-tms-backend.vercel.app/api/projects/manager/${user._id}`,
  //     );
  ...
  async function fetchRequiredDetails() {
    try {
      ...
      const taskRes = await axios.get(`https://cws-tms-backend.vercel.app/manager/emp/task/${managerId}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      
      const allTasksList = [];
      if (taskRes.data.success && taskRes.data.tasksByEmployee) {
        taskRes.data.tasksByEmployee.forEach((employeeGroup) => {
      ...
      setEmployeesTasks(allTasksList);
  ...
  const delayedTasks = employeesTasks.filter((t) => {
  const status = t?.status?.name?.toLowerCase();
  if (status !== "in progress") return false;
  
  const dueDate = t.dateOfExpectedCompletion ? new Date(t.dateOfExpectedCompletion) : null;
  if (!dueDate) return false;
