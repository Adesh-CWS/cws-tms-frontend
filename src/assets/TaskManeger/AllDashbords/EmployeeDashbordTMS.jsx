...
export default function EmployeeDashbordTMS({ user }) {
  ...
  useEffect(() => {
    if (!showTaskModal || !taskModalRef.current) return;
  ...
  const getEffectiveStatus = (task) => {
    if (!task?.dateOfExpectedCompletion) return task.status?.name;
    ...
    return task.status?.name || "Unknown";
  };
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [tasksRes] = await Promise.all([
          axios.get(`https://cws-tms-backend.vercel.app/tasks/assigned/${employeeId}`),
        ]);
        ...
        if (tasksRes.data.tasks) {
          ...
          const upcoming = tasksRes.data.tasks.filter((task) => {
            ...
            const isCompleted = effectiveStatus === "Completed" || (effectiveStatus && effectiveStatus.startsWith("Completed"));
          ...
          const taskinfo =upcoming.map(task => ({ ...task, description: task.taskDescription || task.description || "No description" }));
          setUpcomingTasks(taskinfo);
        }
      } catch (error) {
    ...
    const fetchTaskStats = async () => {
      try {
        ...
        const res = await axios.get(
          `https://cws-tms-backend.vercel.app/tasks/assigned/${user._id}`,
        );
        ...
        // Calculate counts
         const stats = {
          ...
          ongoingTasks: tasks.filter((t) => {
            const effectiveStatus = getEffectiveStatus(t);
            return effectiveStatus === "On Track (In Progress)";
          }).length,
          delayedTasks: tasks.filter((t) => {
            const effectiveStatus = getEffectiveStatus(t);
            return effectiveStatus === "Delayed (In Progress)";
          }).length,
          completedTasks: tasks.filter((t) => {
            const effectiveStatus = getEffectiveStatus(t);
            return effectiveStatus === "Completed" || 
                  (effectiveStatus && effectiveStatus.startsWith("Completed"));
          }).length,
        };
