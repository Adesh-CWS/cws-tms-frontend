...
function EmployeeReportTMS({ employeeId }) {
  ...
  const donutModalRef = useRef(null);
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://cws-tms-backend.vercel.app/tasks/assigned/${employeeId}`)
      .then((res) => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/projects/employee/${employeeId}`,
      );
  ...
  const fetchDelayedTasks = async () => {
    try {
      ...
      const res = await axios.get(`https://cws-tms-backend.vercel.app/tasks/assigned/${employeeId}`);
  
      if (res.data.tasks) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
  
        const delayed = res.data.tasks.filter((task) => {
          if (task.status?.name !== "In Progress") return false;
  ...
  const fetchUpcomingTasks = async () => {
    try {
      ...
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/api/tasks/employee/${employeeId}/upcoming-tasks`,
      );
  ...
  const fetchTeams = async () => {
    try {
      ...
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/api/employee/${employeeId}/teams`,
      );
  ...
  useEffect(() => {
    if (!donutStatus || !donutModalRef.current) return;

    const modal = donutModalRef.current;
