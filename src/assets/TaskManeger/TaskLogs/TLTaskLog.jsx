...
const TLTaskLog = ({ user }) => {
    ...
    for (let i = 0; i < 15; i++) {
      if (isWorkingDay(d)) {
        ...
        const res = await axios.get(
          `https://cws-tms-backend.vercel.app/api/tasklogs/daily-workload?date=${dateStr}`,
  ...
  const fetchWorkload = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/api/tasklogs/daily-workload?date=${selectedDate}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(
        `https://cws-tms-backend.vercel.app/api/tasklogs/tl/${user._id}/logs`,
        {
          headers: {
  ...
  const handleApproveSubmit = async () => {
    try {
      ...
      const res = await axios.put(
        `https://cws-tms-backend.vercel.app/api/tasklogs/approve/${logId}`,
        {
          status: "Approved",
  ...
  const gettingWorkload = async () => {
    try {
      ...
      if (workloadDate) {
        url = `https://cws-tms-backend.vercel.app/api/tasklogs/daily-workload?date=${workloadDate}`;
      } else if (workloadWeek) {
        const weekStartDate = getStartDateOfWeek(workloadWeek);
        url = `https://cws-tms-backend.vercel.app/api/tasklogs/workload/weekly?date=${weekStartDate}`;
      } else if (workloadMonth) {
        const [year, month] = workloadMonth.split("-");
        url = `https://cws-tms-backend.vercel.app/api/tasklogs/workload/monthly?year=${year}&month=${month}`;
      } else {
