...
function EventsAndHolidaysDashboard() {
  ...
  const handleDeleteAnnouncement = async (id) => {
    ...
    try {
      ...
      await axios.delete(`https://cws-tms-backend.vercel.app/announcements/${id}`, {
...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const res = await axios.get("https://cws-tms-backend.vercel.app/announcements/");
  ...
  const handleDeleteHoliday = async (id) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.delete(`https://cws-tms-backend.vercel.app/holidays/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    const fetchEvents = async () => {
      try {
        ...
        const res = await axios.get(
          "https://cws-tms-backend.vercel.app/events-for-employee",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const handleDeleteEvent = async (id) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.delete(`https://cws-tms-backend.vercel.app/events/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
const validAnnouncements = announcementsList.filter((announcement) => {
  ...
  tabIndex={0}
  role="button"
  aria-label={`${event.type} - ${event.name}`}
  onKeyDown={(e) => {
...
>
  ...
  onClick={(e) => {
    ...
    if (
      ...
      alert("Birthday and Anniversary events are not editable.");
      return;
    }
...
onClick={(e) => {
  ...
  tabIndex={0}
  role="button"
  aria-label={`Announcement - ${announcement.name}`}
  onKeyDown={(e) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
