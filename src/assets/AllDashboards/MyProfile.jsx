...
function MyProfile({ user, setUser }) {
  ...
  const handleRemoveImage = async (e) => {
    ...
    try {
      ...
      const res = await axios.delete(
        `https://cws-tms-backend.vercel.app/employees/${user._id}/image`,
      );
    ...
    const fetchProfile = async () => {
      try {
        const res = await axios.get(
          `https://cws-tms-backend.vercel.app/employees/${user._id}`,
        );
        setProfile(res.data);
        setFormData({
          ...res.data,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaving(true);
      // ✅ Only wait for PUT
      await axios.put(`https://cws-tms-backend.vercel.app/employees/${user._id}`, data, {
      ...
      axios
        .get(`https://cws-tms-backend.vercel.app/employees/${user._id}`)
        .then((res) => {
          setProfile(res.data);
  ...
  const renderFileLink = (file) => {
    ...
    return (
      <a
        href={`https://cws-tms-backend.vercel.app/uploads/${file}`}
        target="_blank"
        rel="noreferrer"
      >
        View PDF
                  ...
                  <img
                    src={
                      profile.image?.startsWith("http")
                        ? profile.image
                        : `https://cws-tms-backend.vercel.app/image/${profile.image}`
                    }
                    alt="Profile"
                    style={{
                      width: "100px",
