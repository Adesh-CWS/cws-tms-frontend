...
function EmployeeResignation({ user }) {
  ...
  const fetchEmployeeData = async () => {
    try {
      ...
      const response = await axios.get(
        `https://cws-tms-backend.vercel.app/emp/info/${userData.employeeId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchResignations = async () => {
    try {
      ...
      const response = await axios.get(
        `https://cws-tms-backend.vercel.app/resignation/${userData.employeeId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
...
>
  ...
  style={{ color: "#212529" }}
>
  {selectedResignation.status?.toLowerCase() === "approved"
    ? "Approve By"
    : selectedResignation.status?.toLowerCase() === "rejected"
    ? "Reject By"
    : "Reviewe By"}
</div>
                        <div
