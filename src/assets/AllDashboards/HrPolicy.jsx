...
const API_BASE = "https://cws-tms-backend.vercel.app";
...
function HrPolicy() {
  ...
  useEffect(() => {
    if (statusPolicy) {
      axios
        .get(
          `https://cws-tms-backend.vercel.app/policy/read-employees/${statusPolicy._id}`
        )
        .then((res) => {
  ...
  const fetchReadEmployees = async (policyId) => {
    try {
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/policy/read-employees/${policyId}`
      );
