...
const [regItemsPerPage, setRegItemsPerPage] = useState(5);
...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const leavesRes = await axios.get(
        `https://cws-tms-backend.vercel.app/leave/team-leader/${user._id}`,
      );
      setLeaves(leavesRes.data.leaves || []);
      
      const regRes = await axios.get(
        `https://cws-tms-backend.vercel.app/regularization/team-leader/${user._id}`,
      );
 ...
 const updateLeaveStatus = async (leaveId, status) => {
  ...
  try {
    ...
    await axios.put(
      `https://cws-tms-backend.vercel.app/leave/${leaveId}/status`,
      {
        status,
        userId: user._id,
        role: "Team_Leader",
  ...
  try {
    ...
    await axios.put(
      `https://cws-tms-backend.vercel.app/attendance/regularization/${attendanceId}/status`,
      {
        status,
        actionReason:
...
const totalLeavePages = Math.ceil(filteredLeaves.length / leaveItemsPerPage);
const totalRegPages = Math.ceil(
  filteredRegularizations.length / regItemsPerPage,
);

const indexOfLastLeave = leavePage * leaveItemsPerPage;
const indexOfFirstLeave = indexOfLastLeave - leaveItemsPerPage;

const indexOfLastReg = regPage * regItemsPerPage;
const indexOfFirstReg = indexOfLastReg - regItemsPerPage;
                      ...
                      >
                        {r.regularizationRequest.status === "Pending" ? (
                          <>
                            ...
                            >
                              {isRegActionLoading ? "Approving..." : "Approve"}
                            </button>
                            <button
                              className="btn btn-sm btn-outline-danger"
                              onClick={(e) => { //rutuja added e 07-04-26
                             ...
                             setCurrentRegularizationId(r._id);
                              ...
                              disabled={isRegActionLoading}
                            >
                              {isRegActionLoading ? "Rejecting..." : "Reject"}
                            </button>
                          </>
                        ) : (
                          <span>-</span>
    ...
    >
      <div className="modal-content">
        ...
        <div className="modal-footer">
          ...
          >
            {isRegActionLoading 
              ? (regActionType === "Approved" ? "Approving..." : "Rejecting...")
              : (regActionType === "Approved" ? "Approve" : "Reject")
            }
          </button>
