...
useEffect(() => {
  ...
  const handleDeleteEmployee = async (id) => {
    ...
    try {
      ...
      await axios.delete(
        `https://cws-tms-backend.vercel.app/soft/deleteEmployee/${id}`,
        {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const token = localStorage.getItem("accessToken");
        const res = await axios.get("https://cws-tms-backend.vercel.app/getAllEmployees", {
          headers: { Authorization: `Bearer ${token}` },
        });
        //Geetanjali
  ...
  const handleUpdateManager = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.put(
        `https://cws-tms-backend.vercel.app/users/${selectedEmployee._id}/assign-manager`,
  ...
  const handleUpdateMd = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.put(
        `https://cws-tms-backend.vercel.app/assignMd/${selectedManagerForMd._id}`,
        { mdId: selectedMdId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
