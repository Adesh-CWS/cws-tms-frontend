  ...
  const openEmployeePopup = async (emp) => {
    try {
      ...
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/api/break/admin/${emp._id}?date=${today}`,
        {
          headers: {
...
const fetchLateCheckInHistory = async () => {
  ...
  try {
    const token = localStorage.getItem("accessToken");
    const res = await axios.get(
      `https://cws-tms-backend.vercel.app/attendance/late-checkins`,
      {
        params: {
          from: fromDate,
          to: toDate,
...
<LocalizationProvider dateAdapter={AdapterDayjs}>
  <DatePicker
    ...
    minDate={lateFromDate ? dayjs(lateFromDate) : undefined}
    maxDate={dayjs()}
    onChange={(newValue) =>
      setLateToDate(
        newValue ? newValue.format("YYYY-MM-DD") : ""
...
textField: {
  ...
  const leaveRes = await axios.get(
    "https://cws-tms-backend.vercel.app/attendance/on-leave-employees",
    {
      params: {
