  ...
  const fetchCounts = async () => {
    try {
      const res = await axios.get(
        `https://cws-tms-backend.vercel.app/attendance/regularization/my/${user._id}`,
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const isLeaveDay = leaves.some((leave) => {
        ...
        const isAutoGenerated = leave.approvedByRole === "system" ||leave.reason?.includes("Auto Genrated leave"); // rutuja code for auto genrate leave
        return (
          selected >= from && selected <= to && leave.status !== "rejected"&&!isAutoGenerated //rutuja code 
        );
      });
