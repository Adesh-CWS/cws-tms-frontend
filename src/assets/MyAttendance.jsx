...
function MyAttendance({ employeeId }) {
  ...
  useEffect(() => {
    if (selectedRecord?.leaveRef?.reportingManager) {
      axios
        .get(
          `https://cws-tms-backend.vercel.app/users/${selectedRecord.leaveRef.reportingManager}`,
        )
...
if (selectedStatus === "Today") {
  ...
  //     ds === "Full Day" ||
  //     ds === "Working" ||
  //     ds === "Regularized Full Day"||
  //     ds === "Late Check-In Full Day"
  //   )
  // ) {
  //   matched = true;
  //   return "present-day";
  ...
  if (
    ...
    (ds === "Half Day" || ds === "Regularized Half Day" ||
      ds === "Late Check-In Half Day Penalty"  || ds === "Late Check-In Half Day"
    )
  ) {
    matched = true;
    return "halfday-day";
  ...
  if (
    ...
    (ds === "Late Check-In Full Day" || ds === "Late Check-In Half Day")
  ) {
    matched = true;
  }
  ...
  // Late Half Day
  if (
    selectedStatus === "Late Half Day" &&
    (
      ds === "Late Check-In Half Day Penalty"
    )
  ) {
    matched = true;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const authAxios = axios.create({
        baseURL: "https://cws-tms-backend.vercel.app",
 ...
 useEffect(() => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (rec) {
      ...
      else if (dayStatus === "Half Day" || dayStatus === "Regularized (Half Day)" || dayStatus === "Late Check-In Half Day Penalty") {
        dayPresent = 0;
        halfDayCount += 1;
      }
  ...
  useEffect(() => {
    const fetchBreaks = async () => {
      try {
        const token = localStorage.getItem("accessToken");
        const res = await axios.get("https://cws-tms-backend.vercel.app/api/break/my", {
          headers: { Authorization: `Bearer ${token}` },
        });
...
<div className="col-lg-12" style={{marginLeft:"40px"}}>
...
>
  ...
  >
    <span className="legend-box leave"></span>
    Leave ({summary.leave})
  </span>
  </div>
  <div className="col-6 col-lg-2">
  ...
  >
    <span className="legend-box halfday"></span>
  ...
  <span
    style={{ cursor: "pointer" }}
    onClick={() => setSelectedStatus("Late Check-In")}
  >
    <span
      className="legend-box"
  ...
  <span
    style={{ cursor: "pointer" }}
   onClick={() => setSelectedStatus("Late Half Day")}
  >
    <span
      className="legend-box"
