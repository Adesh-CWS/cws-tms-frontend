...
const actionModalRef = useRef(null);
  ...
  useEffect(() => {
    ...
    axios
      .get("https://cws-tms-backend.vercel.app/me", {
      ...
      for (let leave of leaves) {
        try {
          ...
          const res = await axios.post(
            "https://cws-tms-backend.vercel.app/leave/calculate",
            {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://cws-tms-backend.vercel.app/leaves")
      .then((res) => {
  ...
  const updateStatus = async (leaveId, status) => {
    ...
    try {
      const response = await axios.put(`https://cws-tms-backend.vercel.app/leave/${leaveId}/status`, {
        status,
        userId: user._id,
        role: "admin",
        ...
        prev.map((l) => 
          l._id === leaveId 
            ? { 
                ...
                status: status,
                totalDays: breakdown?.totalDays ?? updatedLeaveFromBackend?.totalDays ?? l.totalDays,
                approvedBy: updatedLeaveFromBackend?.approvedBy,
                actionReason: updatedLeaveFromBackend?.actionReason,
              } 
            : l
        ),
      );
  ...
  const fetchYearlySettings = async () => {
    try {
      const res = await axios.get(
        "https://cws-tms-backend.vercel.app/leave/yearly-settings",
      );
      setData(res.data);
    } catch (err) {
  ...
  const grantYearly = async () => {
    try {
      const res = await axios.post("https://cws-tms-backend.vercel.app/leave/grant-yearly", {
        sl,
        cl,
      });
  ...
  const grantMonthly = async () => {
    try {
      const res = await axios.post(
        "https://cws-tms-backend.vercel.app/leave/grant-monthly",
        {
          sl,
          cl,
        },
  ...
  const fetchLeaveBalance = async () => {
    try {
      const res = await axios.get("https://cws-tms-backend.vercel.app/leave/balance");
  ...
  const HandleDelete = async (leaveId) => {
    ...
    try {
      await axios.delete(`https://cws-tms-backend.vercel.app/leave/${leaveId}`);
...
fetchNotifications();
  ...
  const resetYearlySettings = async () => {
    ...
    try {
      const res = await axios.delete("https://cws-tms-backend.vercel.app/leave/reset-all");
