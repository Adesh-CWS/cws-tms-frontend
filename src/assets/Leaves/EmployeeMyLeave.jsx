...
function EmployeeMyLeave({ user, refreshKey,fetchNotifications}) {
  ...
  useEffect(() => {
    const fetchPublicHolidays = async () => {
      ...
      try {
        const res = await axios.get(`https://cws-tms-backend.vercel.app/getHolidays`);
...
;
          ...
          allIds.map(async (id) => {
            try {
              const r = await axios.get(
                `https://cws-tms-backend.vercel.app/users/${id}`
              );
  ...
  //   try {
  //     const res = await fetch(`https://cws-tms-backend.vercel.app/leave/${id}`, { method: "DELETE" });
  //     if (!res.ok) {
  ...
  const prevLeaves = leaves;
  const prevFiltered = filteredLeaves;


  // remove from both lists
  setLeaves((ls) =>
    ls.filter((x) => x._id !== id)
  );

  setFilteredLeaves((ls) =>
    ls.filter((x) => x._id !== id)
  );
  ...
  try {
    ...
    const res = await fetch(
      `https://cws-tms-backend.vercel.app/leave/${id}`,
      {
        method: "DELETE"
      }
    );
                  ...
                  >
                    ...
                    {l.leaveType !== "LWP" && l.lwpDays > 0 && (
                      <span 
                        ...
                        title={`${l.lwpDays} day${l.lwpDays > 1 ? 's' : ''} without pay`}
                      >
                        {l.lwpDays}D LWP
                      </span>
                    )}
                ...
                <div className="container-fluid">
                  <div className="row mb-2">
                    ...
                    <div className="col-sm-9 col-7">
                      ...
                      {selectedLeave.leaveType !== "LWP" &&
      selectedLeave.lwpDays > 0 && (
        <span
          className="badge ms-2"
          style={{
            backgroundColor: "#dc3545",
