...
function QuickApplyLeave({ user }) {
  ...
  const [leaveBalances, setLeaveBalances] = useState({ CL: 0, SL: 0 });
  ...
  useEffect(() => {
    const fetchManager = async () => {
      ...
      try {
        const res = await axios.get(
          `https://cws-tms-backend.vercel.app/users/${user.reportingManager}`,
        );
        setManager(res.data);
      } catch (err) {
  ...
  const handleApplyClick = () => {
    ...
    let selectedType = formData.leaveType;
    if ((selectedType === "SL" && leaveBalances.SL === 0) || 
    (selectedType === "CL" && leaveBalances.CL === 0)) 
    {
      selectedType = "LWP";
    }
    setForm({
    ...
    if (now < probationEnd) {
      ...
      const types = [];
      if (leaveBalances.CL > 0) types.push("CL");
      if (leaveBalances.SL > 0) types.push("SL");
      types.push("LWP");
      setAvailableLeaveTypes(types);
      
      if (types.length > 0 && types[0] !== "LWP") {
        setForm((prev) => ({ ...prev, leaveType: types[0] }));
      }
    }
 ...
 // -------------------- 🚫 HOLIDAY VALIDATION (NEW) --------------------
    ...
    try {
      ...
      const existingLeavesRes = await axios.get(
        `https://cws-tms-backend.vercel.app/leave/my/${user._id}`,
      );
      ...
      await axios.post("https://cws-tms-backend.vercel.app/leave/apply", {
        employeeId: user._id,
  ...
  useEffect(() => {
    const fetchBalances = async () => {
      if (!user?._id) return;
      try {
        const res = await axios.get(`https://cws-tms-backend.vercel.app/leave/${user._id}`);
        setLeaveBalances({
  ...
  return (
    <div className="card shadow-sm h-100 border-0">
      ...
      <div className="ms-4">
        ...
        >
          <label style={{  marginBottom: "10px" , color: "#3A5FBE", fontWeight: "600" }}>Leave Type</label>
          <select
            name="leaveType"
          ...
          >
            {leaveBalances.SL > 0 && <option value="SL">Sick Leave</option>}
            {leaveBalances.CL > 0 && <option value="CL">Casual Leave</option>}
                      ...
                      {/* Casual Leave */}
                       <div className="form-check d-flex align-items-center" style={{ minWidth: "140px" }}>
                        <input
                          ...
                          onChange={handleChange}
                          disabled={!availableLeaveTypes.includes("CL") || leaveBalances.CL === 0}
                          style={{
                            width: "20px",
                            height: "20px",
                            cursor: leaveBalances.CL === 0 ? "not-allowed" : "pointer",
                            accentColor: "#2E4A8B",
                          }}
                        />
                        <label
                      ...
                      {/* Sick Leave */}
                       <div className="form-check d-flex align-items-center" style={{ minWidth: "140px" }}>
                        <input
                          ...
                          onChange={handleChange}
                          disabled={!availableLeaveTypes.includes("SL") || leaveBalances.SL === 0}
                          style={{
                            width: "20px",
                            height: "20px",
                            cursor: leaveBalances.SL === 0 ? "not-allowed" : "pointer",
                            accentColor: "#2E4A8B",
                          }}
                        />
                        <label
